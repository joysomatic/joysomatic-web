...
export const POST: APIRoute = async ({ request }) => {
  ...
  try {
    ...
    const recipientEmail = (env as any)?.YOUR_PERSONAL_EMAIL || import.meta.env.YOUR_PERSONAL_EMAIL;
    ...
    if (!recipientEmail) {
      clearTimeout(timeoutId);
      return new Response(
        JSON.stringify({ error: 'Missing YOUR_PERSONAL_EMAIL environment variable.' }),
        { status: 500, headers: { 'Content-Type': 'application/json' } }
      );
    }
    ...
    const rawSubject = String(body.subject || body.categoryTitle || 'New Contact Form Submission');
    const rawMessage = String(body.message || body.comments || 'No message content provided.');
